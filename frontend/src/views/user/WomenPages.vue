...
<template>
  <div class="category-page bg-[#fffafb] min-h-screen font-sans text-slate-900" @click="activeDropdown = null">
    <Navbar />
    ...
    <div class="w-full bg-[#0f1117] pt-12 pb-24 px-4 relative overflow-hidden">
      ...
      <div class="absolute -top-24 -right-24 w-96 h-96 bg-rose-600/20 rounded-full blur-[120px]"></div>
      <div class="absolute -bottom-24 -left-24 w-72 h-72 bg-pink-500/10 rounded-full blur-[100px]"></div>

      <div class="max-w-7xl mx-auto w-full relative z-10">
        <nav class="flex text-rose-400/80 text-xs mb-4 uppercase font-bold tracking-[0.2em]">
        ...
        <h1 class="text-4xl md:text-7xl font-black text-white uppercase italic tracking-tighter leading-tight">
          Koleksi <span class="text-transparent bg-clip-text bg-linear-to-r from-rose-400 to-pink-300">Wanita</span>
        </h1>
        <p class="text-slate-400 mt-5 max-w-xl text-sm md:text-base leading-relaxed font-medium">
          Sentuhan elegan untuk setiap penampilan. Temukan perpaduan sempurna antara kenyamanan modern dan siluet
          feminin yang berkelas.
        </p>
        <div class="w-24 h-1.5 bg-linear-to-r from-rose-600 to-transparent mt-8 rounded-full"></div>
      </div>
    </div>

    <main class="max-w-7xl mx-auto px-4 -mt-12 pb-20 relative z-20">
      <section
        class="bg-white p-4 md:p-6 rounded-4xl shadow-2xl shadow-rose-900/5 mb-12 border border-white flex flex-wrap items-center justify-between gap-4">
        <div class="flex flex-wrap items-center gap-3">

          <div class="relative">
            <button @click.stop="activeDropdown = activeDropdown === 'status' ? null : 'status'"
              class="px-5 py-3 rounded-2xl text-xs font-bold flex items-center gap-2 transition-all border border-slate-100 hover:border-rose-500 hover:bg-rose-50"
              :class="{ 'bg-rose-900 border-rose-900 text-white shadow-lg': filters.status }">
            ...
            <div v-if="activeDropdown === 'status'"
              ...
              <div v-for="st in statusOptions" :key="st.value" @click="selectSingleFilter('status', st.value)"
                class="p-3 hover:bg-rose-50 rounded-xl cursor-pointer flex justify-between items-center text-xs font-bold transition-colors">
                {{ st.label }}
                <Check v-if="filters.status === st.value" class="w-4 h-4 text-rose-600" />
              </div>
            </div>
          </div>
          ...
          <div class="relative">
            <button @click.stop="activeDropdown = activeDropdown === 'cat' ? null : 'cat'"
              class="px-5 py-3 rounded-2xl text-xs font-bold flex items-center gap-2 transition-all border border-slate-100 hover:border-rose-500 hover:bg-rose-50"
              :class="{ 'bg-rose-900 border-rose-900 text-white shadow-lg': filters.category }">
            ...
            <div v-if="activeDropdown === 'cat'"
              ...
              <div v-else v-for="cat in categoryOptions" :key="cat.value"
                ...
                class="p-3 hover:bg-rose-50 rounded-xl cursor-pointer flex justify-between items-center text-xs font-bold transition-colors">
                {{ cat.label }}
                <Check v-if="filters.category === cat.value" class="w-4 h-4 text-rose-600" />
              </div>
            </div>
          </div>
          ...
          <div class="relative">
            <button @click.stop="activeDropdown = activeDropdown === 'color' ? null : 'color'"
              class="px-5 py-3 rounded-2xl text-xs font-bold flex items-center gap-2 transition-all border border-slate-100 hover:border-rose-500 hover:bg-rose-50"
              :class="{ 'bg-rose-900 border-rose-900 text-white shadow-lg': filters.colors.length > 0 }">
              Warna <span v-if="filters.colors.length" class="bg-rose-400 text-rose-950 px-1.5 rounded-md ml-1">{{
            ...
            <div v-if="activeDropdown === 'color'" @click.stop
              ...
              <label v-for="color in colorOptions" :key="color.value"
                class="flex items-center gap-3 p-2.5 hover:bg-rose-50 rounded-xl cursor-pointer group">
                <input type="checkbox" :value="color.value" :checked="filters.colors.includes(color.value)"
                  @change="toggleColorFilter(color.value)" class="w-4 h-4 accent-rose-600">
        ...
        <div class="relative">
          <button @click.stop="activeDropdown = activeDropdown === 'sort' ? null : 'sort'"
            class="flex items-center gap-3 px-6 py-3.5 bg-slate-900 text-white rounded-2xl text-xs font-black hover:bg-rose-800 transition-all shadow-xl shadow-slate-900/20">
            <ListFilter class="w-4 h-4 text-rose-400" />
          ...
          <div v-if="activeDropdown === 'sort'"
            ...
            <div v-for="opt in sortOptions" :key="opt.value" @click="selectSingleFilter('sortBy', opt.value)"
              class="p-3 hover:bg-rose-50 rounded-xl cursor-pointer text-xs font-bold flex justify-between items-center transition-colors">
              {{ opt.label }}
              <Check v-if="filters.sortBy === opt.value" class="w-4 h-4 text-rose-600" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section>
        ...
        <div v-else class="py-32 text-center bg-white rounded-[3rem] border border-rose-50 shadow-sm">
          <div class="w-24 h-24 bg-rose-50 rounded-full flex items-center justify-center mx-auto mb-6">
            <Sparkles class="w-10 h-10 text-rose-200" />
          </div>
          <h3 class="text-2xl font-black text-slate-800 uppercase italic tracking-tight">Koleksi Tidak Ditemukan</h3>
          <p class="text-slate-400 mt-2 text-sm">Mungkin Anda bisa mencoba kombinasi filter yang lain.</p>
        </div>

        <div v-if="totalPages > 1" class="flex justify-center items-center gap-4 mt-20">
          <button @click="setPage(currentPage - 1)" :disabled="currentPage === 1"
            class="w-14 h-14 flex items-center justify-center border border-slate-200 rounded-2xl hover:bg-rose-900 hover:text-white disabled:opacity-20 transition-all">
          ...
          <div class="flex items-center gap-2">
            <button v-for="page in totalPages" :key="page" @click="setPage(page)"
              ...
              :class="currentPage === page ? 'bg-rose-900 text-white border-rose-900 shadow-xl shadow-rose-900/30 -translate-y-1' : 'bg-white hover:bg-rose-50 text-slate-400 border-slate-100'">
              {{ page }}
            </button>
          </div>
      ...
      <section class="mt-40 pt-20 border-t border-slate-200">
        <div class="flex items-center justify-between mb-12">
          <h2 class="text-3xl md:text-4xl font-black text-slate-900 uppercase italic tracking-tighter">
            Mungkin Kamu <span class="text-rose-600">Suka</span>
          </h2>
          <div class="h-1 flex-1 bg-rose-50 ml-8 rounded-full opacity-60"></div>
        </div>
...
<style scoped>
.pattern-grid-lg {
  background-image: radial-gradient(circle, #e11d48 0.8px, transparent 0.8px);
